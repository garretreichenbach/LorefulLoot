...
public class Ship extends ManagedUsableSegmentController<Ship> implements AudioEntity {
	...
	public void getRelationColor(RType relation, boolean sameFaction, Vector4f out, float select, float pulse) {
		if(((GameClientState) getState()).getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager().getMapControlManager().isTreeActive() && ((GameClientState) getState()).getPlayer().getFactionId() != this.getFactionId() && ((GameClientState) getState()).getFactionManager().existsFaction(this.getFactionId()) && ((GameClientState) getState()).getFactionManager().getFaction(this.getFactionId()).getFactionMode() != 0) {
			Faction faction = ((GameClientState) getState()).getFactionManager().getFaction(this.getFactionId());
			out.x = faction.getColor().x;
			out.y = faction.getColor().y;
			out.z = faction.getColor().z;
		...
		if(isCoreOverheating()) {
			switch(relation) {
				case ENEMY:
					out.set(ColorPalette.enemyOverheating);
					break;
				case FRIEND:
					out.set(ColorPalette.allyOverheating);
					break;
				case NEUTRAL:
					out.set(ColorPalette.neutralOverheating);
					break;
			}
			if(sameFaction) {
			...
			switch(relation) {
				case ENEMY:
					out.set(ColorPalette.allyUnpoweredDock);
					break;
				case FRIEND:
					out.set(ColorPalette.allyUnpoweredDock);
					break;
				case NEUTRAL:
					out.set(ColorPalette.neutralUnpoweredDock);
					break;
			}
			if(sameFaction) {
			...
			switch(relation) {
				case ENEMY:
					out.set(ColorPalette.enemyTurret);
					break;
				case FRIEND:
					out.set(ColorPalette.allyTurret);
					break;
				case NEUTRAL:
					out.set(ColorPalette.neutralTurret);
					break;
			}
			if(sameFaction) {
			...
			switch(relation) {
				case ENEMY:
					out.set(ColorPalette.enemyDock);
					break;
				case FRIEND:
					out.set(ColorPalette.allyDock);
					break;
				case NEUTRAL:
					out.set(ColorPalette.neutralDock);
					break;
			}
			if(sameFaction) {
			...
			switch(relation) {
				case ENEMY:
					out.set(ColorPalette.enemyShip);
					break;
				case FRIEND:
					out.set(ColorPalette.allyShip);
					break;
				case NEUTRAL:
					out.set(ColorPalette.neutralShip);
					break;
			}
			if(sameFaction) {
	...
	public void onRevealingAction() {
		getManagerContainer().getCloakElementManager().stopCloak(1);
		getManagerContainer().getJammingElementManager().stopJamming(1);
		getManagerContainer().onRevealingAction();
	}
	...
	public boolean allowedType(short type) {
		...
		boolean a = !info.getType().hasParent("spacestation");
		if(info.getType().hasParent("factory")) {
			a = (((GameStateInterface) getState()).getGameState()).isAllowFactoryOnShips();
		}
		a = a || info.getType().hasParent("ship");
		// TODO Temporary overrides for allowing shipyards and shops on ships
		switch(info.id) {
			case 677:
			case 678:
			case 679:
		...
		///
		if(!a && !isOnServer()) {
			((GameClientController) getState().getController()).popupAlertTextMessage(Lng.str("Cannot place\n%s\non a ship!", info.getName()), 0);
		}
			...
			if(ServerConfig.REMOVE_ENTITIES_WITH_INCONSISTENT_BLOCKS.isOn()) {
				for(short s : ElementKeyMap.keySet) {
					if(s != ElementKeyMap.CORE_ID && !allowedType(s) && getElementClassCountMap().get(s) > 0) {
						...
						((GameServerState) getState()).getController().broadcastMessageAdmin(msg, ServerMessage.MESSAGE_TYPE_ERROR);
	...
	public void updateLocal(Timer timer) {
		...
		if(isOnServer() && getFactionId() >= 0 && getAiConfiguration().isActiveAI() && getConfigManager().apply(StatusEffectType.AI_DISABLE, false)) {
		...
		float tRot = getManagerContainer().getThrusterElementManager().rotationBalance * ThrusterElementManager.THRUST_ROT_PERCENT_MULT;
		getOrientationForce().set(Math.min(ThrusterElementManager.MAX_ROTATIONAL_FORCE_X, Math.max(0.001f, ((ThrusterElementManager.BASE_ROTATIONAL_FORCE_X + tRot) / Math.max(1f, FastMath.pow(railController.railedInertia.x, ThrusterElementManager.INTERTIA_POW))))), Math.min(ThrusterElementManager.MAX_ROTATIONAL_FORCE_Y, Math.max(0.001f, ((ThrusterElementManager.BASE_ROTATIONAL_FORCE_Y + tRot) / Math.max(1f, FastMath.pow(railController.railedInertia.y, ThrusterElementManager.INTERTIA_POW))))), Math.min(ThrusterElementManager.MAX_ROTATIONAL_FORCE_Z, Math.max(0.001f, ((ThrusterElementManager.BASE_ROTATIONAL_FORCE_Z + tRot) / Math.max(1f, FastMath.pow(railController.railedInertia.z, ThrusterElementManager.INTERTIA_POW))))));
		...
		if(flagNameChange || nameTag.length() == 0 || lastFactionId != getFactionId() || (getFactionId() != 0 && lastFaction != ((FactionState) getState()).getFactionManager().getFaction(getFactionId()))) {
	...
	public String toNiceString() {
		Fleet fleet;
		if(getNetworkObject() != null && getNetworkObject().getDebugState().get().length() > 0) {
			String cloak = "(stealth:" + getStealthStrength() + ")";
			return nameTag + cloak + (isScrap() ? " (decayed)" : "") + ((fleet = getFleet()) != null ? " FLT[" + fleet.getName() + "]" : "") + getNetworkObject().getDebugState().get() + "\n[CLIENTAI " + (getAiConfiguration().isActiveAI() ? "ACTIVE" : "INACTIVE") + "] " + getAiConfiguration().getAiEntityState();
		} else {
			if(!isOnServer()) {
	...
	public void updateFromNetworkObject(NetworkObject o, int senderId) {
		...
		if(!isOnServer()) {
			if(!((NetworkShip) o).onHitNotices.getReceiveBuffer().isEmpty()) {
				getManagerContainer().onHitNotice();
			}
		}
	}
	...
	public void updateToNetworkObject() {
		...
		if(isOnServer()) {
			getNetworkObject().jamming.set(getManagerContainer().isJamming());
			getNetworkObject().cloaked.set(getManagerContainer().isCloaked());
	...
	public void onAttachPlayer(PlayerState playerState, Sendable detachedFrom, Vector3i where, Vector3i parameter) {
		...
		getManagerContainer().thrustConfiguration.onAttachPlayer(playerState);
		if(!isOnServer()) {
			GameClientState s = (GameClientState) getState();
			if(s.getPlayer() == playerState) {
				Controller.getAudioManager().switchSound(this, "0022_ambience loop - interior cockpit (loop)", 1, 1);
	...
	public void onDetachPlayer(PlayerState playerState, boolean hide, Vector3i parameter) {
		if(!isOnServer()) {
			...
			if(s.getPlayer() == playerState && ((GameClientState) getState()).getPlayer() == playerState) {
				...
				Controller.getAudioManager().switchSound(this, "0022_spaceship user - small engine thruster loop", 1, 1);
			}
		} else {
		}
		getManagerContainer().thrustConfiguration.onDetachPlayer(playerState, getAttachedPlayers());
		refreshNameTag();
	...
	protected void readExtraTagData(Tag t) {
		if(t.getType() == Type.STRUCT) {
			...
			if(ts.length > 1 && ts[1].getType() == Type.BYTE) {
				//old version
				getManagerContainer().thrustConfiguration.readFromOldTag(ts);
			}
		}
	}
	...
	protected Tag getExtraTagData() {
		return new Tag(Type.STRUCT, null, new Tag[] {new Tag(Type.BYTE, null, (byte) 0), //placeholder for compatibility
			FinishTag.INST,});
	}
	...
	public void onSegmentAddedSynchronized(Segment s) {
		if(!modifiedBleuprintTriggered && spawnedFrom != null && isOnServer() && getLastModifier().length() == 0) {
			double f = ((Float) ServerConfig.MODIFIED_BLUEPRINT_TOLERANCE.getCurrentState()).doubleValue();
			...
			if((blueprintPrice + (blueprintPrice * f)) < getElementClassCountMap().getCurrentPrice()) {
				...
				if((blueprintPrice) > getElementClassCountMap().getCurrentPrice() - (blueprintPrice * f)) {
					getManagerContainer().getShieldAddOn().flagCompleteLoad();
				}
			}
		}
	}
	...
	public boolean isCloakedFor(SimpleTransformableSendableObject<?> viewer) {
		if(railController.getRoot().isUsingPowerReactors()) {
			if(railController.isRoot()) {
				if(viewer != null) {
					return !viewer.canSeeStructure(this);
				} else {
		...
		if(getNetworkObject().cloaked.get()) {
			return true;
	...
	public boolean isJammingFor(SimpleTransformableSendableObject<?> viewer) {
		if(railController.getRoot().isUsingPowerReactors()) {
			if(railController.isRoot()) {
				if(viewer != null) {
					return !viewer.canSeeIndicator(this);
				} else {
		...
		if(getNetworkObject().jamming.get()) {
			return true;
		} else if(railController.isDockedAndExecuted()) {
			//recusively check the chain
			...
			//recusively check the chain
			return getDockingController().getDockedOn().to.getSegment().getSegmentController().isJammingFor(viewer);
		}
		return false;
	}
	...
	public boolean isSalvagableFor(Salvager harvester, String[] cannotHitReason, Vector3i position) {
		...
		if(!(harvester instanceof AbstractCharacter<?>) && !isCoreOverheating() && !isMinable()) {
			cannotHitReason[0] = Lng.str("You can only salvage overheating ships.");
			return false;
		}
		if(core.equals(position)) {
		...
		if(harvester.getFactionId() == getFactionId() && ((p instanceof PlayerState && !allowedToEdit((PlayerState) p)) || harvester.getOwnerFactionRights() < getFactionRights())) {
		...
		if(isHomeBase() || (getDockingController().isDocked() && getDockingController().getDockedOn().to.getSegment().getSegmentController().isHomeBaseFor(getFactionId()))) {
		...
		if(!(harvester instanceof AbstractCharacter<?>) && getAiConfiguration().isActiveAI()) {
	...
	public Ship copy(String name, int sector, Transform tr, String spawner, int factionId, List<Ship> spawned, EntityCopyData d) {
		...
		final Ship newShip = EntityRequest.getNewShip(getState(), UID, sector, name, mat, -2, -2, -2, 2, 2, 2, spawner, false);
		...
		newShip.getAiConfiguration().setFrom(this.getAiConfiguration());
		Tag tModules = getManagerContainer().getModuleTag();
		Tag tInTag = getManagerContainer().getInventoryTag();
		...
		//		}
		segBuf.iterateOverNonEmptyElement(new SegmentBufferIteratorInterface() {
			@Override
			public boolean handle(Segment s, long lastChanged) {
				...
				if(segmentData != null) {
					...
					SegmentData dataCopy;
					if(segmentData instanceof SegmentDataIntArray) {
						dataCopy = new SegmentDataIntArray(segmentData);
					} else {
						dataCopy = new SegmentDataIntArray(!isOnServer());
		...
		//only load filters...
		newShip.getManagerContainer().loadInventoriesFromTag = false;
		newShip.getManagerContainer().fromTagModule(tModules, 0);
		newShip.getManagerContainer().fromTagInventory(tInTag);
		Tag aiTag = getAiConfiguration().toTagStructure();
		newShip.getAiConfiguration().fromTagStructure(aiTag);
		spawned.add(newShip);
		String rlStr;
		...
		for(RailRelation r : railController.next) {
			Ship copyOfDock = ((Ship) r.docked.getSegmentController()).copy(name + rlStr + i, sector, tr, spawner, factionId, spawned, d);
	...
	private void handleCopyUIDTranslation() {
		getManagerContainer().getTransporter().getElementManager().handleCopyUIDTranslation();
		for(RailRelation r : railController.next) {
	...
	public boolean isAutomaticReactivateDampeners() {
		return getManagerContainer().thrustConfiguration.isAutomaticReacivateDampeners();
	}

	public boolean isAutomaticDampeners() {
		return getManagerContainer().thrustConfiguration.isAutomaticDampeners();
	}
	...
	public float getLinearDamping() {
		...
		if((dampen > 1f || getManagerContainer().thrustConfiguration.isAutomaticDampeners()) && !getManagerContainer().getThrusterElementManager().isUsingThrust()) {
	...
	public boolean isNPCFactionControlledAI() {
		...
		return FactionManager.isNPCFaction(getFactionId()) && getAiConfiguration().isActiveAI();
	}
