package videogoose.lorefulloot.utils;
...
import videogoose.lorefulloot.data.item.meta.MetaItem;
...
public class MiscUtils {
	 ...
	 */
	public static void wreckEntity(ManagedUsableSegmentController<?> entity) {
		(new Thread(() -> {
			try {
				for(int j = 0; j < 5; j++) {
					int explosionCap = 10;
	...
	private static long getRandomIndex(SegmentController entity, int attempts) {
		...
		if(attempts < 30) {
			if(segment == null) {
				return getRandomIndex(entity, attempts + 1);
			} else {
